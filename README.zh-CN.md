...
- **文献引用真实可验证。** paper-search MCP 检索 Crossref/arXiv/PubMed 记录，内联 DOI 渲染为可点击的来源——绝不编造参考文献。
- **架构级 local-first。** 工作区就是你机器上的普通文件夹；除非你通过已配置的模型发送内容，或显式调用文献检索等外部服务，否则项目文件不会离开本机。支持 Ollama、LM Studio 等纯本地端点，由控制平面管理的连接器目标域名会记录到本地出站审计中。
