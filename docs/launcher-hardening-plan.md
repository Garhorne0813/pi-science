...
- Use one end-to-end configurable `PI_SCIENCE_STARTUP_TIMEOUT_SECONDS` deadline with a 90-second default across sequential control-plane and frontend readiness. Detached startup uses the same outer deadline and rolls back the supervisor, PID file, and owned listeners on failure.
...
- Serialize cooperating installers with a same-directory lock, revalidate immediately before commit, and hard-link the prepared same-directory file into place so a file, directory, or symlink created after validation is never overwritten. Clean lock/snapshot/temp files on failure.
...
- Persisted owner PID/process-start identity or lease architecture for cross-process job orphan healing. Windows CI exposed a live job being marked failed at the 15-second orphan grace boundary; focused local runs pass, and fixing the durable ownership model requires a separate approved control-plane design change rather than a timeout increase or retry.
- Dependency upgrades.
