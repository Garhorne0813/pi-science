...
9. Per-file lock release silently swallowed sidecar deletion failures. On Windows, a transient `EPERM`/`EBUSY` could leave a live-PID lock behind, causing later research-loop state writes to wait until their test deadline. Serialization reduced contention but did not fix this durable lock lifecycle defect.
...
- Run launcher tests only in Ubuntu CI. Windows CI continues to validate the cross-platform application but is not evidence of native shell-launcher support. PowerShell checks fail immediately after each native command. Server files remain serialized on the two-core Windows runner, while per-file lock release now uses ownership tokens, bounded retry for transient `EPERM`/`EBUSY`, and surfaced permanent failures so stale sidecars cannot masquerade as scheduling noise.
...
| Durable file locks | Transient Windows-style unlink retry, surfaced permanent release failure without queue hang, replacement-token preservation, stale-lock recovery |
