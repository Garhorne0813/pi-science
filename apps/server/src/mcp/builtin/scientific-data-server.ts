...
switch (domain) {
  ...
  case "protein_records":
    server.registerTool("search_uniprot_proteins", { title: "Search UniProt Proteins", description: "Search UniProtKB when no exact accession is available, with controlled organism, review-status, isoform, sorting, and pagination parameters. Do not call this alongside get_uniprot_entry for an accession.", inputSchema: uniprotSearchInput, annotations }, async (input) => result(await searchUniprotProteins(input)));
    server.registerTool("get_uniprot_entry", { title: "Get UniProt Entry", description: "Retrieve compact UniProtKB protein, gene, organism, function, and sequence metadata directly by exact accession. Do not also call search_uniprot_proteins.", inputSchema: uniprotEntryInput, annotations }, async (input) => result(await getUniprotEntry(input)));
    server.registerTool("get_uniprot_sequence", { title: "Get UniProt Sequence", description: "Retrieve a standalone UniProtKB FASTA record by accession only when sequence output is explicitly needed. Do not call it when get_uniprot_entry already uses include_sequence=true.", inputSchema: uniprotSequenceInput, annotations }, async (input) => result(await getUniprotSequence(input)));
    break;
