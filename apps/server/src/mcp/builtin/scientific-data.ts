...
const connectorIds: Record<string, string> = { literature_graph: "mcp_builtin_literature_graph", clinical_trials: "mcp_builtin_clinical_trials", structures: "mcp_builtin_structures", genes_ontologies: "mcp_builtin_genes_ontologies", genomes: "mcp_builtin_genomes", cellguide: "mcp_builtin_cellguide" };
async function getJson(domain: string, url: URL, dependencies: ScientificDependencies) { return requestJson(domain, url, dependencies, { headers: { accept: "application/json" } }); }
async function requestJson(domain: string, url: URL, dependencies: ScientificDependencies, init: RequestInit = {}): Promise<unknown> { return JSON.parse(await requestText(domain, url, dependencies, init)); }
async function requestText(domain: string, url: URL, dependencies: ScientificDependencies, init: RequestInit = {}): Promise<string> {
...
function envelope(source: string, request: Json, records: Json[], total: number | null, dependencies: ScientificDependencies, next?: unknown) { return { source, retrieved_at: (dependencies.now?.() ?? new Date()).toISOString(), request, count: records.length, total, ...(next ? { next_page_token: next } : {}), records }; }
function singleEnvelope(source: string, request: Json, record: Json, dependencies: ScientificDependencies) { return { source, retrieved_at: (dependencies.now?.() ?? new Date()).toISOString(), request, record }; }
function object(value: unknown): Json { return value && typeof value === "object" && !Array.isArray(value) ? value as Json : {}; }
function numeric(value: unknown): number | null { const result = Number(value); return Number.isFinite(result) ? result : null; }
