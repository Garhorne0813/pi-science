...
describe("runtime route boundaries", () => {
  it("keeps scientific routes on Python and control routes on Node", () => {
    ...
    expect(runtimeOwner("/api/bookmarks")).toBe("node-control-plane");
    expect(runtimeOwner("/api/attention")).toBe("node-control-plane");
    expect(runtimeOwner("/api/sessions/session-1/read-state")).toBe("node-control-plane");
