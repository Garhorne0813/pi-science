...
export function registerSettingsRoutes(app: FastifyInstance, nodeSessionService: NodeSessionService, settingsStore: SettingsStore, modelResources?: ModelResourceService, mcp?: McpConnectorService): void {
  ...
  app.addHook("preValidation", async (request) => {
    ...
    try {
      ...
      const discovered = await discoverProvider(String(body.base_url), String(body.api_key ?? ""), String(body.api ?? "openai-completions"), privateProviderAccessEnabled(config.allow_private_providers), 2_000);
  ...
  app.get("/api/settings/mcp", async (request) => { if (mcp) { const result = await mcp.list(query(request, "cwd", ".")); return { servers: result.connectors.filter((item) => item.binding?.enabled).map((item) => item.connector_id), configured: result.connectors.map((item) => item.connector_id), config_path: null, deprecated: true }; } const config = await load(); const { definitions, source } = await resolveMcpConfig({ explicitPath: typeof config.mcp_config_path === "string" ? config.mcp_config_path : undefined }); const configured = Object.keys(definitions); const enabled = Array.isArray(config.mcp_servers) ? config.mcp_servers.filter((id) => configured.includes(id)) : configured; return { servers: enabled, configured, config_path: source }; });
  app.put<{ Params: { server_id: string } }>("/api/settings/mcp/:server_id", async (request, reply) => { const on = String((request.query as { enabled?: string }).enabled ?? "true") !== "false"; if (mcp) { const cwd = query(request, "cwd", "."); const item = (await mcp.list(cwd)).connectors.find((connector) => connector.connector_id === request.params.server_id || connector.name === request.params.server_id); if (!item) return reply.code(404).send({ error: "MCP connector not found" }); await mcp.setBinding(item.connector_id, { enabled: on, include_tools: item.binding?.include_tools ?? [], exclude_tools: item.binding?.exclude_tools ?? [], approval_mode: item.binding?.approval_mode ?? "ask", ...(item.binding ? { revision: item.binding.revision } : {}) }, cwd); return { ok: true, server: item.connector_id, enabled: on, deprecated: true }; } await mutate((config) => { const enabled = new Set(Array.isArray(config.mcp_servers) ? config.mcp_servers : []); if (on) enabled.add(request.params.server_id); else enabled.delete(request.params.server_id); config.mcp_servers = [...enabled].sort(); }); return respondWithReload(reply, { ok: true, server: request.params.server_id, enabled: on }); });
