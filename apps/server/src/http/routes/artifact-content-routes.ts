...
async function readFileChunk(handle: FileHandle, maxBytes: number): Promise<Buffer> {
...
export function registerArtifactContentRoutes(app: FastifyInstance): void {
  app.get<{ Params: { sha256: string } }>("/api/artifacts/content/:sha256/serve", async (request, reply) => {
    try {
      ...
      return reply.type(type).send(resolved.handle.createReadStream({ autoClose: true }));
    } catch (error) {
  ...
  app.get<{ Params: { sha256: string } }>("/api/artifacts/content/:sha256", async (request, reply) => {
    try {
      ...
      const resolved = await resolveArtifactContent(cwd, requestedPath, request.params.sha256);
      try {
        if (maxBytes === null && resolved.size > MAX_INLINE_BYTES) return reply.code(400).send({ error: `Artifact content is too large to read (${resolved.size} bytes).` });
        const data = maxBytes === null ? await resolved.handle.readFile() : await readFileChunk(resolved.handle, maxBytes);
        ...
        return {
          ...
          ...(data.byteLength < resolved.size ? { truncated: true } : {}),
        };
      } finally {
