...
export class JobCoordinator {
  ...
  async submit(cwd: string, body: Record<string, unknown>): Promise<JobRecord> {
    ...
    const executionRoot = surface.startsWith("research") ? metadataRoot(cwd) : resolve(cwd);
    const executionRelative = relative(executionRoot, executionCwd);
    const executionInsideRoot = !isAbsolute(executionRelative) && executionRelative !== ".." && !executionRelative.startsWith(`..${process.platform === "win32" ? "\\" : "/"}`);
    if (!executionInsideRoot && !(surface.startsWith("research") && executionCwd === resolve(cwd))) throw new Error(`execution cwd escapes the ${surface.startsWith("research") ? "workspace state" : "workspace"}`);
    const requestedJobId = surface === "conversation" && typeof body.client_job_id === "string" && /^job_[0-9a-f]{16}$/.test(body.client_job_id)
      ? body.client_job_id : null;
    const jobId = requestedJobId ?? `job_${randomUUID().replaceAll("-", "").slice(0, 16)}`;
    if (requestedJobId && await this.get(cwd, jobId)) throw new Error("Conversation job ID already exists");
    // Research commands are generated by an agent. Resolve and probe the OS
  ...
  private async run(record: JobRecord, environment: NodeJS.ProcessEnv, spawnCommand: string[]): Promise<void> {
    const path = this.jobPath(record.cwd, record.job_id);
    ...
    try {
      ...
      const spawned = this.repository
        ? await this.repository.locked(record.job_id, async () => {
          ...
          spawnedProcess = this.processes.spawn(record.job_id, spawnCommand, record.execution_cwd ?? record.cwd, environment, Math.max(1, Number(record.requirement.timeout_seconds ?? 3600)) * 1000);
      ...
      // Sandy reserves 130 for its own Job Object timeout (child exit codes
      // above 124 are not passed through), which may fire before our watchdog.
      const sandboxTimedOut = record.environment.sandbox_backend === "appcontainer" && result.code === 130;
      const nextStatus = this.cancelled.has(record.job_id) ? "cancelled" : result.timed_out || sandboxTimedOut ? "timed_out" : result.code === 0 ? "succeeded" : "failed";
