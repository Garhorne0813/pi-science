...
it("routes the Bash tool and direct Bash through sandbox jobs", async () => {
  ...
  vi.stubGlobal("fetch", vi.fn(async (url: string, init: RequestInit = {}) => {
    ...
    pollCount += 1;
    return new Response(JSON.stringify({ job_id: "job-1", status: "succeeded", return_code: 0, stdout: `ok-${pollCount}\n`, stderr: "" }), { status: 200 });
  }));
  ...
  expect(result.details).toEqual({ sandboxed: true });
...
it("cancels a job when abort arrives during creation", async () => {
  ...
  vi.stubGlobal("fetch", vi.fn(async (url: string, init: RequestInit = {}) => {
    ...
    return new Response(JSON.stringify({ job_id: "job-1", status: "cancelled", stdout: "", stderr: "" }), { status: 200 });
  }));
  const events = new Map<string, any>();
  registerSandbox({ registerTool: () => undefined, on: (name: string, handler: any) => events.set(name, handler) });
