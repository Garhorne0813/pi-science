...
import { workspaceStateRoot } from "../../../storage/persistence.js";

const cleanup: string[] = [];
const originalHome = process.env.PI_SCIENCE_HOME;
const originalStateRoot = process.env.PI_SCIENCE_STATE_ROOT;

afterEach(async () => {
  await Promise.all(cleanup.splice(0).map((path) => rm(path, { recursive: true, force: true })));
  if (originalHome === undefined) delete process.env.PI_SCIENCE_HOME;
  else process.env.PI_SCIENCE_HOME = originalHome;
  if (originalStateRoot === undefined) delete process.env.PI_SCIENCE_STATE_ROOT;
  else process.env.PI_SCIENCE_STATE_ROOT = originalStateRoot;
});
...
describe("Pi-Science MCP runtime extension", () => {
  it("resolves the managed snapshot from each session cwd", async () => {
    ...
    process.env.PI_SCIENCE_STATE_ROOT = process.env.PI_SCIENCE_HOME;
    const workspace = join(root, "workspace");
    await mkdir(workspace, { recursive: true });
    await mkdir(workspaceStateRoot(workspace), { recursive: true });
    await writeFile(join(workspaceStateRoot(workspace), "mcp-runtime.json"), JSON.stringify({
      version: 1,
  ...
  it("materializes credential references only inside the Pi process", async () => {
    ...
    await writeFile(join(workspaceStateRoot(workspace), "mcp-runtime.json"), snapshot, "utf8");
