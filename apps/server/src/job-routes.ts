...
import { JobCoordinator, publicJobRecord, type JobRequirement } from "./job-coordinator.js";
...
export function registerJobRoutes(app: FastifyInstance, jobs: JobCoordinator): void {
  ...
  app.post("/api/jobs", async (request, reply) => { const cwd = await workspace(request, reply); if (!cwd) return; try { return publicJobRecord(await jobs.submit(cwd, (request.body ?? {}) as Record<string, unknown>)); } catch (error) { return reply.code(400).send({ error: error instanceof Error ? error.message : String(error) }); } });
