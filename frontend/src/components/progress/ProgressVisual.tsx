...
export function ProgressVisual({ slot, config, state = "running", text = "Generating response" }: { slot: ProgressSlot; config: ProgressAppearance; state?: "running" | "waiting" | "completed"; text?: string }) {
  ...
  if (definition.kind === "inline") {
    ...
    return variant ? <InlineLoader variant={variant} speed={speed} color={color} paused={paused} label={slot === "waiting" ? text : undefined} /> : null;
  }
  if (definition.kind === "text") {
    const variant = TEXT_VARIANTS[definition.id as keyof typeof TEXT_VARIANTS];
    return variant ? <TextLoader text={text} variant={variant} speed={speed} color={color} paused={paused} aria-label={text} /> : null;
  }
  if (definition.kind === "image") {
    const variant = IMAGE_VARIANTS[definition.id as keyof typeof IMAGE_VARIANTS];
    return variant ? <ImageLoader variant={variant} speed={speed} color={color} paused={paused} size="7rem" radius="0.5rem" label={text} /> : null;
  }
