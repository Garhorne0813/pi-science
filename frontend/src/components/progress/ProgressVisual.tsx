...
/** The single effective-motion policy: "off" always pauses, "system" follows
 *  the subscribed OS preference, "full" is an explicit override. */
export function effectiveMotionPaused(motion: ProgressAppearance["motion"], reduced: boolean): boolean {
  return motion === "off" || (motion === "system" && reduced);
}
...
export function ProgressVisual({ slot, config, state = "running", activityState, text = "Generating response", compact = false }: { slot: ProgressSlot; config: ProgressAppearance; state?: "running" | "waiting" | "completed"; activityState?: ProgressActivityState; text?: string; compact?: boolean }) {
  ...
  // One place decides the effective motion: "off" always pauses, "system"
  // follows the subscribed OS preference, "full" is an explicit override.
  const reduced = useReducedMotion();
  const paused = effectiveMotionPaused(config.motion, reduced);
