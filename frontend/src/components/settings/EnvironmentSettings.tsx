  ...
  return (
    <section className="py-5">
      <div className="mb-4 flex items-start justify-between gap-4">
        <div><h2 className="text-sm font-medium text-text">{t("settings.environments.title")}</h2><p className="mt-1 text-xs text-muted">{t("settings.environments.description")}</p></div>
        <button type="button" onClick={() => void create()} disabled={busy !== null} className="flex h-9 items-center gap-1.5 rounded-input bg-accent px-3 text-xs font-medium text-accent-fg disabled:opacity-50">{busy === "create" ? <Loader2 size={13} className="animate-spin" /> : <Plus size={13} />} {t("settings.environments.new")}</button>
      </div>
      ...
      <div className="overflow-hidden rounded-card border border-faint">
        {environments.length === 0 ? <p className="p-4 text-xs text-muted">{t("settings.environments.empty")}</p> : environments.map((environment) => {
          ...
          return <div key={environment.revision_id} className="flex items-center gap-3 border-b border-faint p-3 last:border-b-0">
            ...
            {active ? <span className="flex items-center gap-1 text-xs text-ok"><Check size={13} /> {t("settings.environments.active")}</span> : workspaceCwd && environment.status === "ready" ? <button type="button" disabled={busy !== null} onClick={() => void bind(environment.revision_id)} className="rounded-input border border-border px-3 py-1.5 text-xs text-text hover:bg-surface-2 disabled:opacity-50">{busy === environment.revision_id ? t("settings.environments.switching") : t("settings.environments.use")}</button> : environment.status === "failed" || environment.status === "archived" ? <button type="button" disabled={busy !== null} onClick={() => void remove(environment.revision_id)} className="flex items-center gap-1 rounded-input border border-error/30 px-2.5 py-1.5 text-xs text-error hover:bg-error/10 disabled:opacity-50">{busy === environment.revision_id ? <Loader2 size={12} className="animate-spin" /> : <Trash2 size={12} />} {t("common.delete")}</button> : <span className="text-xs text-muted">{environment.status}</span>}
          </div>;
        })}
      </div>
    </section>
