...
export function MCPTab({ workspaceCwd }: { workspaceCwd: string | null }) {
  ...
  const connectorRead = useQuery({ ...mcpConnectorsQuery(workspaceCwd ?? "", t("settings.mcpPage.loadError")), enabled: Boolean(workspaceCwd) });
  ...
  const select = async (connector: McpConnector) => { setSelectedId(connector.connector_id); if (!workspaceCwd) return; try { setTools((await settingsApi.mcpTools(workspaceCwd, connector.connector_id)).tools); } catch { setTools([]); } };
  ...
  if (!workspaceCwd) return <p className="pt-card text-sm text-muted">{t("settings.mcpPage.workspaceRequired")}</p>;
