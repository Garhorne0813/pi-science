...
export function MCPTab({ workspaceCwd }: { workspaceCwd: string | null }) {
  ...
  const connectorRead = useQuery(mcpConnectorsQuery(workspaceCwd, t("settings.mcpPage.loadError")));
  ...
  const toggle = async (connector: McpConnector, enabled: boolean) => {
    ...
    const key = mcpConnectorsKey(workspaceCwd);
    const previous = queryClient.getQueryData(key);
    queryClient.setQueryData<{ connectors: McpConnector[] }>(key, (current) => current ? { ...current, connectors: current.connectors.map((item) => item.connector_id === connector.connector_id ? { ...item, binding: item.binding ? { ...item.binding, enabled } : item.binding } : item) } : current);
    setBusy(connector.connector_id); setError(null);
    try { await settingsApi.updateMcpBinding(workspaceCwd, connector.connector_id, { enabled, include_tools: connector.binding?.include_tools ?? [], exclude_tools: connector.binding?.exclude_tools ?? [], approval_mode: connector.binding?.approval_mode ?? "ask", ...(connector.binding ? { revision: connector.binding.revision } : {}) }); await connectorRead.refetch(); }
  ...
  const create = async () => {
    ...
    const body: McpConnectorCreate = {
      ...
      credential_ref: null, enable_for_project: true,
    ...
    await mutate("new", async () => { await settingsApi.createMcp(workspaceCwd, body); setAdding(false); setForm(blank); });
  };

  const select = async (connector: McpConnector) => { setSelectedId(connector.connector_id); try { setTools((await settingsApi.mcpTools(workspaceCwd, connector.connector_id)).tools); } catch { setTools([]); } };
  const importLegacy = async () => { if (!workspaceCwd) return; await mutate("import", async () => { const preview = await settingsApi.previewMcpImport(workspaceCwd); const names = preview.entries.filter((item) => item.importable && !item.conflict).map((item) => item.name); if (!names.length) throw new Error(t("settings.mcpPage.importNone")); const result = await settingsApi.commitMcpImport(workspaceCwd, names); if (result.failed.length) throw new Error(t("settings.mcpPage.importFailed", { error: result.failed.map((item) => `${item.name}: ${item.error}`).join("; ") })); }); };
  ...
  return <div className="space-y-card pt-card">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-ui-body text-muted">{t("settings.mcpPage.canonicalDescription")}</p><p className="mt-1 text-ui-caption text-muted">{t("settings.mcpPage.stateHint")}</p>{!workspaceCwd && <p className="mt-2 rounded-input bg-surface-2 px-3 py-2 text-ui-caption text-muted">{t("settings.mcpPage.workspaceRequired")}</p>}</div>{workspaceCwd && <div className="flex gap-2">{legacyCount > 0 && <button type="button" onClick={() => void importLegacy()} disabled={busy !== null} className="inline-flex items-center gap-1 rounded-input border border-border px-3 py-2 text-xs"><Upload size={14} />{t("settings.mcpPage.importLegacy", { count: legacyCount })}</button>}<button type="button" onClick={() => setAdding((value) => !value)} className="inline-flex items-center gap-1 rounded-input bg-accent px-3 py-2 text-xs text-white"><Plus size={14} />{t("settings.mcpPage.addConnector")}</button></div>}</div>
    ...
    <section className="ui-card-flat overflow-hidden rounded-card"><table className="w-full table-fixed text-left"><thead className="border-b border-border bg-surface-2/50"><tr><th className="px-4 py-2.5 text-xs text-muted">{t("settings.mcpPage.tableName")}</th><th className="hidden px-4 py-2.5 text-xs text-muted md:table-cell">{t("settings.mcpPage.tableDescription")}</th><th className="px-4 py-2.5 text-xs text-muted">{t("settings.mcpPage.tableStatus")}</th><th className="hidden px-4 py-2.5 text-xs text-muted md:table-cell">{t("settings.mcpPage.tableTools")}</th><th className="px-4 py-2.5 text-xs text-muted">{t("settings.mcpPage.tableTest")}</th><th className="px-4 py-2.5 text-xs text-muted">{t("settings.mcpPage.tableEnabled")}</th></tr></thead><tbody className="divide-y divide-border">{connectors.length ? connectors.map((connector) => <McpRow key={connector.connector_id} connector={connector} selected={selectedId === connector.connector_id} busy={busy === connector.connector_id} actionsEnabled={Boolean(workspaceCwd)} onSelect={() => void select(connector)} onProbe={() => { if (workspaceCwd) void mutate(connector.connector_id, async () => { const result = await settingsApi.probeMcp(workspaceCwd, connector.connector_id); if (result.error) throw new Error(result.error); }); }} onToggle={(enabled) => void toggle(connector, enabled)} />) : <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-muted">{t("settings.mcpPage.empty")}</td></tr>}</tbody></table></section>
    {selected && <aside className="ui-card-flat rounded-card p-4"><div className="flex items-start justify-between"><div><h3 className="font-semibold text-text">{selected.display_name}</h3><p className="mt-1 font-mono text-xs text-muted">{selected.endpoint_url || [selected.command, ...selected.args].filter(Boolean).join(" ") || selected.socket_path}</p></div>{workspaceCwd && selected.source !== "builtin" && <button type="button" aria-label={t("settings.mcpPage.delete", { name: selected.display_name })} onClick={() => { if (window.confirm(t("settings.mcpPage.deleteConfirm", { name: selected.display_name }))) void mutate(selected.connector_id, async () => { await settingsApi.deleteMcp(workspaceCwd, selected.connector_id); setSelectedId(null); }); }} className="text-error-text"><Trash2 size={16} /></button>}</div>
