...
describe("SkillsTab", () => {
  it("lists skills and marks project skills with a badge", async () => {
    ...
    expect(within(tables[0]).getByText("alpha")).toBeInTheDocument();
    expect(within(tables[1]).getByText("beta")).toBeInTheDocument();
    expect(within(tables[0]).queryByRole("columnheader", { name: "Actions" })).not.toBeInTheDocument();
    expect(within(tables[1]).getByRole("columnheader", { name: "Actions" })).toBeInTheDocument();
