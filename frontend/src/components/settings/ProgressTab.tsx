  ...
  return <div className="space-y-4">
    <section className="overflow-hidden rounded-card border border-faint bg-surface-2/40">
      ...
      <div className="divide-y divide-faint">
        <SettingRow label={t("settings.progress.preset")}><SettingsSelectMenu variant="compact" ariaLabel={t("settings.progress.preset")} value={draft.preset} options={["quiet", "research", "science", "custom"].map((value) => ({ value, label: t(`settings.progress.preset.${value}`) }))} onSelect={(value) => updatePreset(value as ProgressAppearance["preset"])} /></SettingRow>
        <SettingRow label={t("settings.progress.motion")}><SettingsSelectMenu variant="compact" ariaLabel={t("settings.progress.motion")} value={draft.motion} options={["system", "full", "off"].map((value) => ({ value, label: t(`settings.progress.motion.${value}`) }))} onSelect={(value) => update({ motion: value as ProgressAppearance["motion"] })} /></SettingRow>
        <SettingRow label={t("settings.progress.color")}><div className="flex items-center gap-2"><SettingsSelectMenu variant="compact" ariaLabel={t("settings.progress.color")} value={draft.colorMode} options={["semantic", "custom"].map((value) => ({ value, label: t(`settings.progress.color.${value}`) }))} onSelect={(value) => update({ colorMode: value as ProgressAppearance["colorMode"] })} />{draft.colorMode === "custom" && <input aria-label={t("settings.progress.customColor")} type="color" value={draft.customColor || "#679efe"} onChange={(event) => update({ customColor: event.target.value })} className="h-8 w-10 cursor-pointer rounded-input border border-border bg-transparent p-0.5" />}</div></SettingRow>
