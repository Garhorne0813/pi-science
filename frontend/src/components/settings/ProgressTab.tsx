  ...
  return <div className="space-y-4">
    <section className="overflow-hidden rounded-card border border-faint bg-surface-2/40">
      ...
      <div className="divide-y divide-faint">
        <SettingRow label={t("settings.progress.preset")}><SettingsSelectMenu ariaLabel={t("settings.progress.preset")} value={draft.preset} options={["quiet", "research", "science", "custom"].map((value) => ({ value, label: t(`settings.progress.preset.${value}`) }))} onSelect={(value) => updatePreset(value as ProgressAppearance["preset"])} /></SettingRow>
        <SettingRow label={t("settings.progress.motion")}><SettingsSelectMenu ariaLabel={t("settings.progress.motion")} value={draft.motion} options={["system", "full", "off"].map((value) => ({ value, label: t(`settings.progress.motion.${value}`) }))} onSelect={(value) => update({ motion: value as ProgressAppearance["motion"] })} /></SettingRow>
        <SettingRow label={t("settings.progress.color")}><div className="flex items-center gap-2"><SettingsSelectMenu ariaLabel={t("settings.progress.color")} value={draft.colorMode} options={["semantic", "custom"].map((value) => ({ value, label: t(`settings.progress.color.${value}`) }))} onSelect={(value) => update({ colorMode: value as ProgressAppearance["colorMode"] })} />{draft.colorMode === "custom" && <input aria-label={t("settings.progress.customColor")} type="color" value={draft.customColor || "#679efe"} onChange={(event) => update({ customColor: event.target.value })} className="h-8 w-10 cursor-pointer rounded-input border border-border bg-transparent p-0.5" />}</div></SettingRow>
    ...
    <section className="overflow-hidden rounded-card border border-faint bg-surface-2/40">
      ...
      <div className="divide-y divide-faint">
        {SLOTS.map((slot) => <SettingRow key={slot.id} label={t(slot.labelKey)} preview={<ProgressVisual slot={slot.id} config={draft} state={slot.id === "completed" ? "completed" : "running"} text={slot.sample} />}><SettingsSelectMenu ariaLabel={t(slot.labelKey)} value={draft.patterns[slot.id]} options={patternsForSlot(slot.id).map((pattern) => ({ value: pattern.id, label: t(pattern.labelKey) }))} onSelect={(value) => updatePattern(slot.id, value as ProgressAppearance["patterns"][ProgressSlot])} /></SettingRow>)}
      </div>
    </section>
  </div>;
}
...
function SettingRow({ label, children, preview }: { label: string; children: ReactNode; preview?: ReactNode }) {
  return <div className="flex min-h-14 flex-wrap items-center justify-between gap-3 px-4 py-2"><span className="text-ui-label font-medium text-text">{label}</span><div className="flex min-w-0 items-center gap-3">{preview && <span className="flex min-h-7 min-w-12 items-center justify-center text-muted">{preview}</span>}{children}</div></div>;
}
