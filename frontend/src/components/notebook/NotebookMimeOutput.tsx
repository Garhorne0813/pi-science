export function NotebookMimeOutput({ mime, label }: { mime: Record<string, string>; label: string }) {
  ...
  return (
    <div className="space-y-2">
      {(png || jpeg) && <img src={`data:${png ? "image/png" : "image/jpeg"};base64,${png || jpeg}`} alt={label} className="max-h-[520px] max-w-full rounded-input bg-white object-contain dark:bg-[var(--surface-inset)]" />}
      {svg && <img src={`data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg)}`} alt={label} className="max-h-[520px] max-w-full rounded-input bg-white object-contain dark:bg-[var(--surface-inset)]" />}
      ...
      {html && (
        <iframe
          ...
          className="h-64 w-full rounded-input border border-faint bg-[var(--bg)]"
        />
      )}
    </div>
  );
