...
describe("activity over time (PRD v1.2 §26/§28)", () => {
  it("streams the latest prose, keeps one phase row, and settles the answer at session.idle", async () => {
    stubWorkspace();
    await useRuntimeStore.getState().connect("/workspace", SESSION);
    vi.useFakeTimers();
    try {
      ...
      rerender(view());
      // Until a later tool supersedes it, the newest prose is rendered from
      // the first text update so direct answers do not wait for session.idle.
      ...
      emit("text.updated", { partId: "m2", text: "这是最终" });
      rerender(view());
      expect(screen.getByText("这是最终")).toBeInTheDocument();
      emit("text.updated", { partId: "m2", text: "回答。" });
      rerender(view());
