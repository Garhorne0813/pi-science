import { BarChart3, Code2, FileText, FlaskConical } from "lucide-react";
import { cn } from "../../lib/cn";

const starters = [
  { icon: BarChart3, label: "Analyze data", desc: "Load a CSV, NetCDF, or FITS file and find patterns.", prompt: "Analyze the dataset in this directory and give me key statistical insights." },
  { icon: FileText, label: "Write report", desc: "Generate a scientific report from the findings.", prompt: "Write a scientific report based on the data analysis results." },
  { icon: Code2, label: "Run Python", desc: "Execute code in an interactive notebook session.", prompt: "Write and run a Python script to process the data files in this workspace." },
  { icon: FlaskConical, label: "Run experiment", desc: "Design and execute a computational experiment.", prompt: "Design an experiment to test the hypothesis and run the analysis." },
];
      ...
      <div className="mt-7 w-full max-w-[500px] rounded-card border border-border bg-surface shadow-card">
        {starters.map((starter, index) => (
          <button key={starter.label} type="button" onClick={() => onPick(starter.prompt)} disabled={disabled} className={cn("group flex w-full items-center gap-3.5 px-4 py-3.5 text-left disabled:cursor-wait disabled:opacity-50", !disabled && "hover:bg-surface-2", index > 0 && "border-t border-border", index === 0 && "rounded-t-card", index === starters.length - 1 && "rounded-b-card")}>
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-surface-2 text-accent ring-1 ring-border"><starter.icon size={16} strokeWidth={1.75} /></span>
