...
export function ResearchLoopDraftCard({ draft, busy, onChange, onCancel, onConfirm }: { draft: ResearchLoopDraft; busy: boolean; onChange: (draft: ResearchLoopDraft) => void; onCancel: () => void; onConfirm: () => void }) {
  const { t } = useTranslation();
  return <section className="rounded-card border border-accent/30 bg-accent/5 p-4 shadow-card">
    <div className="flex items-start justify-between gap-3"><div><h3 className="flex items-center gap-2 text-sm font-semibold text-text"><FlaskConical size={15} className="text-accent" /> {t("research.confirmTitle")}</h3><p className="mt-1 text-xs leading-5 text-muted">{t("research.confirmDescription")}</p></div><button type="button" onClick={onCancel} disabled={busy} aria-label={t("research.cancelSetup")} className="min-h-9 min-w-9 text-muted hover:text-text"><X size={15} /></button></div>
    <div className="mt-4 grid gap-3 sm:grid-cols-2">
      <Field label={t("research.title")} wide><input value={draft.title} onChange={(event) => onChange({ ...draft, title: event.target.value })} className="field" /></Field>
      <Field label={t("research.objective")} wide><textarea value={draft.objective} onChange={(event) => onChange({ ...draft, objective: event.target.value })} rows={3} className="field py-2" /></Field>
      <Field label={t("research.metric")}><input value={draft.metric} onChange={(event) => onChange({ ...draft, metric: event.target.value })} className="field" /></Field>
      <Field label={t("research.direction")}><select value={draft.direction} onChange={(event) => onChange({ ...draft, direction: event.target.value as ResearchLoopDraft["direction"] })} className="field"><option value="maximize">{t("research.maximize")}</option><option value="minimize">{t("research.minimize")}</option></select></Field>
      <Field label={t("research.optionalTarget")}><input type="number" value={draft.target ?? ""} onChange={(event) => onChange({ ...draft, target: event.target.value === "" ? undefined : Number(event.target.value) })} className="field" /></Field>
      <Field label={t("research.maxCandidates")}><input type="number" min={1} max={100} value={draft.maxCandidates} onChange={(event) => onChange({ ...draft, maxCandidates: Math.max(1, Number(event.target.value) || 1) })} className="field" /></Field>
      <Field label={t("research.maxRuntime")}><input type="number" min={1} max={1440} value={Math.ceil(draft.maxWallSeconds / 60)} onChange={(event) => onChange({ ...draft, maxWallSeconds: Math.max(60, (Number(event.target.value) || 1) * 60) })} className="field" /></Field>
    </div>
    <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={onCancel} disabled={busy} className="min-h-9 rounded-input border border-border px-3 text-xs text-muted">{t("common.cancel")}</button><button type="button" onClick={onConfirm} disabled={busy || !draft.title.trim() || !draft.objective.trim() || !draft.metric.trim()} className="flex min-h-9 items-center gap-1.5 rounded-input bg-accent px-3 text-xs font-medium text-accent-fg disabled:opacity-50">{busy ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />} {t("research.createStart")}</button></div>
  </section>;
}
...
function Field({ label, wide, children }: { label: string; wide?: boolean; children: React.ReactNode }) { return <label className={cn("text-xs text-muted", wide && "sm:col-span-2")}>{label}<div className="mt-1 [&_.field]:min-h-9 [&_.field]:w-full [&_.field]:rounded-input [&_.field]:border [&_.field]:border-border [&_.field]:bg-surface [&_.field]:px-3 [&_.field]:text-sm [&_.field]:text-text">{children}</div></label>; }
