...
import type { ExperienceRecord, ResearchLoopDetail, ResearchTaskType } from "../../lib/knowledge";
...
export interface ResearchLoopDraft { taskType: Extract<ResearchTaskType, "research_loop" | "optimize">; title: string; objective: string; successCriterion: string; planSteps: string[]; metric: string; direction: "maximize" | "minimize"; maxCandidates: number; maxWallSeconds: number; benchmarkPath: string }
...
export function ResearchLoopDraftCard({ draft, busy, onCancel, onConfirm, onBenchmarkChange, onMetricChange, onDirectionChange }: { draft: ResearchLoopDraft; busy: boolean; onCancel: () => void; onConfirm: () => void; onBenchmarkChange: (path: string) => void; onMetricChange: (metric: string) => void; onDirectionChange: (direction: "maximize" | "minimize") => void }) {
  ...
  return <section className="ui-card-accent rounded-card p-4">
    ...
    <div className="mt-3 grid gap-3 sm:grid-cols-2">
      <label className="text-xs text-muted">{t("research.metric")}<input className="mt-1 w-full rounded-input border border-border bg-surface px-3 py-2 font-mono text-xs text-text" value={draft.metric} onChange={(event) => onMetricChange(event.target.value)} disabled={busy} /></label>
      <label className="text-xs text-muted">{t("research.direction")}<select className="mt-1 w-full rounded-input border border-border bg-surface px-3 py-2 text-xs text-text" value={draft.direction} onChange={(event) => onDirectionChange(event.target.value as "maximize" | "minimize")} disabled={busy}><option value="maximize">{t("research.maximize")}</option><option value="minimize">{t("research.minimize")}</option></select></label>
    </div>
    <div className="mt-3 grid gap-3 sm:grid-cols-2">
    ...
    <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={onCancel} disabled={busy} className="min-h-9 rounded-input border border-border px-3 text-xs text-muted">{t("common.cancel")}</button><button type="button" onClick={onConfirm} disabled={busy || !draft.benchmarkPath.trim() || !draft.metric.trim()} className="flex min-h-9 items-center gap-1.5 rounded-input bg-accent-fill px-3 text-xs font-medium text-accent-fg disabled:opacity-50">{busy ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />} {t("research.createStart")}</button></div>
  </section>;
}

export function ResearchLoopStatusCard({ loop, candidates, busy, onRefresh, onAction, onOpenDetails }: { loop: ResearchLoopDetail; candidates: ExperienceRecord[]; busy: boolean; onRefresh: () => void; onAction: (action: "pause" | "resume" | "cancel") => void; onOpenDetails: () => void }) {
  const { t } = useTranslation();
  ...
  return <section className="ui-card-flat rounded-card border-accent/30 p-4">
    ...
    {loop.status === "pausing" && <p className="mt-2 text-xs text-warn-text">{t("research.pausingHint")}</p>}
    {loop.status === "needs_attention" && <p className="mt-2 text-xs text-error-text">{t("research.needsAttention", { reason: loop.stop_reason })}</p>}
    <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4"><Status label={t("research.candidates")} value={`${candidates.length}/${loop.budget.max_candidates}`} /><Status label={t("research.currentStage")} value={t(`research.status.${current?.status ?? "planning"}`, { defaultValue: current?.status ?? t("research.planning") })} /><Status label={latestMetric?.[0] ?? t("research.latestMetric")} value={latestMetric?.[1]?.value == null ? "—" : String(latestMetric[1].value)} /><Status label={t("research.baseline")} value={latestMetric?.[0] && loop.baseline?.[latestMetric[0]] != null ? String(loop.baseline[latestMetric[0]]) : "—"} /></div>
    <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-muted"><span>{loop.execution_isolation?.available ? t("research.isolationActive", { backend: loop.execution_isolation.backend }) : t("research.isolationUnavailable")}</span>{loop.decision?.next_strategy && <span>{t("research.nextStrategy")}: {loop.decision.next_strategy}</span>}</div>
