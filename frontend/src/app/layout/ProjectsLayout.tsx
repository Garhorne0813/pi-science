...
import { PanelLeft, Settings, MessageSquare, Plus, Trash2, GitFork, GitBranch, FolderOpen, ArrowLeft, Puzzle, FileText, BookOpen, Play, Inbox, FlaskConical, type LucideIcon } from "lucide-react";
      ...
      <div className="mb-1 flex h-tool items-center justify-between px-2">
        ...
        {(attentionCounts.needs_you > 0 || attentionCounts.running > 0 || attentionCounts.unread > 0 || attentionCounts.plan_ready > 0) && (
          <span className="flex items-center gap-1 text-ui-micro text-muted" aria-label={t("conversation.attentionSummary")}>
            ...
            {attentionCounts.plan_ready > 0 && <span className="rounded-full bg-info/15 px-1.5 py-px text-[9px] text-info">{attentionCounts.plan_ready} {t("conversation.attentionPlanReady")}</span>}
          </span>
        )}
        <IconButton
          icon={Plus}
