...
import { ResearchDecisionPanel } from "../../components/knowledge/ResearchDecisionPanel";
...
export function ResearchPage() {
  ...
  return <WorkspacePage><WorkspacePageHeader title={t("nav.research")} description={t("research.pageDescription")} actions={<><button type="button" onClick={() => navigate(`/workspace/${encodeURIComponent(cwd)}`)} className="min-h-9 rounded-input bg-accent-fill px-3 text-xs text-accent-fg">{t("research.startFromConversation")}</button><WorkspacePageRefreshButton label={t("common.refresh")} loading={loading} onClick={() => void loopsResult.refetch()} /></>} />
    ...
    {detail && <ResearchDecisionPanel detail={detail} />}
