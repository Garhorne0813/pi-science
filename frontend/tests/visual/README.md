...
`playwright.visual.config.ts` defines five projects:
...
- `toHaveScreenshot` uses `maxDiffPixelRatio: 0.005`, `animations: "disabled"`
  and `caret: "hide"`.
- Baselines live in `tests/visual/__screenshots__/<project>/`. Every baseline
  update must be reviewed: a `test:visual:update` run with unrelated pixel
  drift means the fixture is not deterministic — investigate before committing.
...
- Projects page, workspace landing hero, collapsed sidebar rail.
