...
[BioMysteryBench](https://huggingface.co/datasets/Anthropic/BioMysteryBench-preview)
— the 5-problem preview (open) and the 90-problem full set
([BioMysteryBench-full](https://huggingface.co/datasets/Anthropic/BioMysteryBench-full),
gated). It downloads the pinned dataset revision once, runs model
...
uv run python fetch_dataset.py            # preview (default), downloads to .cache/, verifies sha256
...
### 1b. Full dataset (90 problems, gated)

1. Open the
   [dataset page](https://huggingface.co/datasets/Anthropic/BioMysteryBench-full)
   and accept the terms with your HuggingFace account (`gated: auto`).
2. Export a read token for that account — the harness reads it **only** from
the `HF_TOKEN` environment variable and never writes it anywhere:

```bash
HF_TOKEN=hf_... uv run python fetch_dataset.py --dataset full
# debug: fetch problems.csv + data for the first 2 problems only
HF_TOKEN=hf_... uv run python fetch_dataset.py --dataset full --limit 2
```

The full set downloads `problems.csv` (verified against the pinned sha256) and
one zip per problem (`data/hbNNN.zip`, listed via the HuggingFace tree API;
observed sha256 recorded in `.cache/fetch-manifest.json`). Running the
assessment afterwards attaches a local data-file hint to every prompt so
models with file access can analyze the attached data.
...
# Same, but on the full dataset cache:
uv run python run_eval.py --data .cache/problems.json --limit 10
...
`meta.json` (includes the `dataset` tag so full-set runs are identifiable).
...
tag, problem count). Dataset files themselves are pinned by revision + sha256 in
`manifest.json` (the gated full set additionally carries `gated: true` and its
own pinned revision/files entry); observed hashes are written to
`.cache/fetch-manifest.json`.
