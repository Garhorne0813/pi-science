...
description: Audit a scientific report, manuscript, or analysis in the workspace for traceability — resolving citations, flagging numbers with no source, and checking figures against the code that generated them. Emits a structured review block. Verifies traceability, never "correctness".
version: 0.1.0
license: Apache-2.0
category: review
requirements:
  ...
  - name: network
    kind: service
    optional: true
    description: Crossref, arXiv, and PubMed APIs for citation resolution.
risk: low
---
...
Run the bundled extractor first — it pulls the text plus concrete citation
identifiers and quantitative claims deterministically, so you audit real
identifiers, not ones recalled from memory:

```bash
python skills/traceability-review/pdf_extract.py MANUSCRIPT.pdf
```
